    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.ADMIN_ONLY:
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply_sticker("CAACAgIAAxkBAANwYjSpXQmVTeVKf8IMaxvnI2Fdl-cAAvYXAAIHo6FJpE_AM_jxlf8jBA")
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgIAAxkBAANwYjSpXQmVTeVKf8IMaxvnI2Fdl-cAAvYXAAIHo6FJpE_AM_jxlf8jBA", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('👨🏻‍🔧بیا اینجا', url='https://t.me/DIGRM')]]))
    await delete_messages([m, k])
